...
| **Embeddings Model** | `all-MiniLM-L6-v2` (Local) | A lightweight, fast embedding model that runs entirely on the CPU via `sentence-transformers`. Avoids the latency overhead of calling an external OpenAI embedding API during live voice calls. |
